...
def test_압축률이_높은_창은_소음으로_본다():
    ...
    """
    segments = [seg("정상적인 회의 발언입니다", 1.23), seg("이곳은 한국의 한 식당입니다", 5.30)]
...
def test_같은_말이_연달아_반복되면_압축률과_무관하게_소음이다():
    segments = [seg("곱창 곱창", 1.0, i) for i in range(4)]
...
def test_구두점_차이는_같은_말로_본다():
    segments = [seg("감사합니다", 1.0, 0), seg("감사합니다.", 1.0, 1), seg("감사합니다!", 1.0, 2)]
...
def test_한_세그먼트_안의_낱말_되풀이도_잡는다():
    segments = [seg("매일 매일 매일 매일 매일 매일 매일", 1.0)]
